<template>
  ...
  >
    <p class="col-span-5 break-all">{{ item.userRequestId }}</p>
  ...
  <!-- Mobile layout -->
  <div
    class="sm:hidden bg-bg1 p-2 rounded-md shadow-sm text-sm text-text1 space-y-0.5"
  >
    <p>
      <span class="font-semibold">{{ $t("request.id") }}:</span>
      {{ item.userRequestId }}
    </p>
    <p>
      <span class="font-semibold">{{ $t("request.lastUpdated") }}:</span>
    ...
    <p>
      <span class="font-semibold">{{ $t("request.mushrooms") }}:</span>
      {{ item.numberOfMushrooms }}
    </p>
  </div>
</template>
